...
export const dragBoundFunc=(pos,size) => {
...
export const dragBoundFuncRectangle=(pos,size,value) => {
  const radius = 5; // Radius of the circles
  const stageWidth = size.width;
  const stageHeight = size.height;
  const minX = radius;
  const maxX = stageWidth - value.width - radius; 
  const minY = radius;
  const maxY = stageHeight - value.height - radius; 
  const x = Math.min(maxX, Math.max(minX, pos.x));
  const y = Math.min(maxY, Math.max(minY, pos.y));
  return { x, y };
}
